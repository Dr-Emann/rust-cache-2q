...
///
/// The cache is split into 3 sections, A1in, A1out and Am. A1in contains the most recently added entries.
...
/// When an entry is requested and not found, but its key is found in A1out, an entry is pushed to the front of Am.
///
/// # Examples
///
/// ```
...
impl<K: Eq, V> Cache<K, V> {
    ...
    pub fn get<Q: ?Sized>(&mut self, key: &Q) -> Option<&V>
        where K: Borrow<Q>,
              Q: Eq
    {
        if let Some(&CacheEntry{ref value, ..}) = self.a1_in.iter().find(|entry| entry.key.borrow() == key) {
            Some(value)
    ...
    pub fn get_mut<Q: ?Sized>(&mut self, key: &Q) -> Option<&mut V>
        where K: Borrow<Q>,
              Q: Eq
    {
        if let Some(&mut CacheEntry{ref mut value, ..}) = self.a1_in.iter_mut().find(|entry| entry.key.borrow() == key) {
            Some(value)
        } else if let Some(i) = self.am.iter().position(|entry| entry.key.borrow() == key) {
            let old = self.am.remove(i).unwrap();
            self.am.push_front(old);
    ...
    pub fn entry(&mut self, key: K) -> Entry<K, V> {
        ...
        if let Entry::Occupied(OccupiedEntry { ref mut cache, kind: OccupiedKind::Frequent(ref mut i), ..}) = entry {
    ...
    pub fn remove<Q: ?Sized>(&mut self, key: &Q) -> Option<V>
        where K: Borrow<Q>,
              Q: Eq,
    {
        if let Some(i) = self.a1_in.iter().position(|entry| entry.key.borrow() == key) {
...
impl<'a, K: 'a + fmt::Debug, V: 'a + fmt::Debug> fmt::Debug for OccupiedEntry<'a, K, V> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("OccupiedEntry")
            ...
            .field("kind", & if let OccupiedKind::Frequent(_) = self.kind { "frequent" } else { "recent" })
            .finish()
    }
}
...
impl<'a, K: 'a + fmt::Debug, V: 'a + fmt::Debug> fmt::Debug for VacantEntry<'a, K, V> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_struct("VacantEntry")
            .field("key", self.key())
            .field("remembered", & if let VacantKind::Remembered(_) = self.kind { true } else { false })
            .finish()
    }
}
...
pub struct Iter<'a, K: 'a, V: 'a> {
    inner: iter::Map<iter::Chain<vec_deque::Iter<'a, CacheEntry<K, V>>, vec_deque::Iter<'a, CacheEntry<K, V>>>, fn(&'a CacheEntry<K, V>) -> (&K, &V)>
}
